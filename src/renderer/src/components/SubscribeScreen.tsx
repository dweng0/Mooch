...
import handCardIcon from '../assets/proposed_images/Hands_General_WEBM/Hand_Holding_Card.webm'
  ...
  return (
    <div className="h-full flex flex-col bg-white/90 rounded-2xl backdrop-blur-sm border border-gray-200 overflow-hidden shadow-2xl">
      ...
      <div className="drag-region px-4 py-3 border-b border-gray-200">
        <h1 className="text-sm font-semibold text-gray-900 text-center">Mooch</h1>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-6 py-4 gap-5">
        ...
        <div className="w-full">
          ...
          <button
            onClick={onRefresh}
            className="w-full mt-1.5 text-xs text-gray-500 hover:text-gray-700 transition-colors cursor-pointer"
          >
        ...
        <div className="w-full">
          <p className="text-xs text-gray-500 mb-2 flex items-center gap-1.5">
            <video src={handCardIcon} autoPlay loop muted playsInline className="h-8 w-8" />
          ...
          <div className="relative mb-2">
            <select
              ...
              className="w-full appearance-none bg-gray-100 text-gray-700 text-xs rounded-lg px-3 py-2.5 border border-gray-200 outline-none focus:border-blue-500 cursor-pointer pr-8"
            >
              {PROVIDERS.map((p) => (
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-gray-100 text-gray-700 text-xs rounded-lg px-3 py-2.5 border border-gray-200 outline-none focus:border-blue-500 placeholder-gray-400"
            />
            <button
              onClick={handleSave}
              disabled={!apiKey.trim() || saving}
              className="flex-shrink-0 px-3 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-200 disabled:text-gray-500 disabled:cursor-not-allowed text-white text-xs rounded-lg transition-colors cursor-pointer flex items-center gap-1.5"
            >
              {saving ? (
